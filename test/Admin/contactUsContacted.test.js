...
describe('Test the contact us contacted with filter POST api', () => {
    it("It should response 200 to the POST method", async () => {

        const response = await request(baseUrl).post('api/v1/admin/getContactUsContactedWithFilter').set('token', token).send({
            name: "kk",
            page: 1,
            limit: 10,
            sortOrder: -1,
    ...
    test.each(data)("It should response 400 to the POST method", async (singleData) => {
        ...
        const response = await request(baseUrl).post('api/v1/admin/getContactUsContactedWithFilter').set('token', token).send(singleData)
