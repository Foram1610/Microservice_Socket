...
describe("Test the provider update profile image API", () => {

    it("It should response 200 to the post method", async () => {

        const response = await request(baseUrl).post('api/v1/provider/updateProfileImage').set('accept-language', 'en').set('token', jsonData.token).attach('image', imagefile)
